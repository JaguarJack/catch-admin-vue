    ...
    <el-row :gutter="12">
      ...
      <el-col :span="18">
        <div class="filter-container">
          <el-input v-model="searchUsers.username" placeholder="用户名" clearable class="filter-item form-search-input" />
          <el-input v-model="searchUsers.email" placeholder="邮箱" clearable class="filter-item form-search-input" />
          <el-select v-model="searchUsers.status" clearable placeholder="请选择" class="filter-item" style="margin-right: 5px">
            <el-option value="1" label="启用" />
            <el-option value="2" label="禁用" />
          </el-select>
          <el-button class="filter-item" type="primary" icon="el-icon-search" @click="handleSearchUser">
            搜索
          </el-button>
          <el-button class="filter-item" icon="el-icon-refresh" @click="resetThenRefresh">
            重置
          </el-button>
...
export default {
  ...
  methods: {
    ...
    getDepartmentUsers(data, node, self) {
      this.searchUsers.department_id = data.id
      this.getUsers()
    },
    // 禁用/启用 用户
    ...
    getUsers() {
      this.$http.get('users', { params: this.searchUsers }).then(response => {
    ...
    multiDeleteUsers() {
      if (this.selectUsers.length) {
        this.$http.delete('users/' + this.selectUsers.join(',')).then(response => {
          this.$message({
            message: '删除成功',
            type: 'success'
          })
